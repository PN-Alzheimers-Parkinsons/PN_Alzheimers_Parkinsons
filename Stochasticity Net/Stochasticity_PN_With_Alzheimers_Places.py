...
class BaseArc:
    def __init__(self, place, arc_weight):#remove set arc_weight = 1
        ...
        Args: 
            ...
            coefficient_scalar: Brandon: the coefficient that will be randomised to scale the arc_weights by a random amount
...
class Transition:
    ...
    def fire(self):
        ...
        for arc2 in self.out_arcs.union(self.in_arcs):
            arc2.coefficient_scalar= random_integer#added
            #print(arc2.coefficient_scalar)
        #for in_arc in self.in_arcs:
            #print(in_arc.allowed_firing())#debug
        firing_allowed = all(in_arc.allowed_firing() for in_arc in self.in_arcs)
        #all is an inbuilt function which returns true, if all elements of the iterable are true
        firing_not_inhibited = all(inhib_arc.allowed_firing() for inhib_arc in self.inhib_arcs)
        #print("firing_allowed", firing_allowed) #debugging
        #print("firing_not_inhibited",firing_not_inhibited) #debugging
        if firing_allowed and firing_not_inhibited:
            ...
            for arc in self.out_arcs.union(self.in_arcs): 
                arc.fire()
                #print(self.out_arcs.union(self.in_arcs))#brandon4
...
class PetriNet:
    ...
    The initial conditions of the net is sorted in petri_net. This 
    ...
    def run(self, number_of_steps, print_stats = False):
        ...
        for step in range(number_of_steps):
            ...
            print(self.timeseries_mean[step])#Brandon2
        if print_stats:
...
#Brandon so above you are setting the attributes of the class PetriNetModel
    @staticmethod
    def make_copy_of(petri_net_model):
        ...
        for t in petri_net_model.transitions.values():
            ...
            input_place_ids = [arc.place.place_id for arc in t.in_arcs] #i dont think arc needs to be properly defined, arc is like the x in a for loop. place_id is like h2 o2
            output_place_ids = [arc.place.place_id for arc in t.out_arcs]
            inhib_place_ids = [arc.place.place_id for arc in t.inhib_arcs]

            input_arc_weights = [arc.arc_weight for arc in t.in_arcs] # this is how the input_arc_weights are applied to the actual arc_weights 
            output_arc_weights = [arc.arc_weight for arc in t.out_arcs]
            inhib_arc_weights = [arc.arc_weight for arc in t.inhib_arcs]
#have a new object here that has the min and max values for coefficient_scalers
            #input_coefficient_scalar = [arc.coefficient_scalar for arc in t.in_arcs]
    ...
    def add_transition(self, transition_id, label, input_place_ids, input_arc_weights, output_place_ids, output_arc_weights, inhib_place_ids=[], inhib_arc_weights=[]):
        ...
        if transition_id in self.transitions.keys():
            ...
            transition = Transition([InArc(place, weight) for (place, weight) in zip(input_places, input_arc_weights)], # I don't think weight needs to be defined because InArc previously took 2 variables anyway, the place and the arc_weight, if you put it in the right order, that's all that matters.
    ...
    def run_one_step(self):
        ...
        """
        t = random.choice(list(self.transitions.values())) #BSL: So transitions is a dictionary, and values returns the values of the transitions dictionary. t will therefore be a randomly selected transition.
        successful_firing = t.fire()
        self.successful_firings.append(successful_firing)#successful_firings is a list and you are expanding the list every time step.

        return [place.tokens for place in self.places.values()] #tokens is an attribute of the dictionary place, and you are returning the tokens for every place that the dictionary place has.
        #print(self.places.values(), "yoyo") #brandon3
